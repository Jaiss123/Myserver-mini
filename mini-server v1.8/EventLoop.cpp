...
void* EventLoop::runAt(Timestamp when, IRun* pRun)
{
    return _pTimerQueue->addTimer(pRun, when, 0.0);
}

void* EventLoop::runAfter(double delay, IRun* pRun)
{
    return _pTimerQueue->addTimer(pRun, Timestamp::nowAfter(delay), 0.0);
}

void* EventLoop::runEvery(double interval, IRun* pRun)
{
    return _pTimerQueue->addTimer(pRun, Timestamp::nowAfter(interval), interval);
}

void EventLoop::cancelTimer(void* timerId)
{
