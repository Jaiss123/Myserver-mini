...
/// @return the process unique Timer* of the timer

void* TimerQueue::addTimer(IRun* pRun, Timestamp when, double interval)
{
    Timer* pTimer = new Timer(when, pRun, interval); //Memory Leak !!!
    _pLoop->queueLoop(_addTimerWrapper, pTimer);
    return static_cast<void*>(pTimer);
}

void TimerQueue::cancelTimer(void* timerId)
{
