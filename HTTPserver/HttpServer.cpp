...
HttpServer::HttpServer(EventLoop* pLoop, int threadnums, int iothreadnums)
    :_pLoop(pLoop)
    ,_pServer(pLoop,iothreadnums)
    ,_timer(-1)
    ,_index(0)
    ,_threadnums(threadnums)
    ,connectionBuckets_(8)                //断开连接时间为: 8
{
    _pServer.setCallback(this);
    connectionBuckets_.resize(8);

    //pLoop->runEvery(1.0, this);    //8s断开连接
}
...
void HttpServer::onConnection(TcpConnection* pCon)
{
    ...
    EntryPtr entry(new Entry(pCon));
    connectionBuckets_.back().insert(entry);
    WeakEntryPtr weakEntry(entry);
    pCon->setweakEntry(weakEntry);        //将entry的弱指针通过connection保存
    pCon->setContext(HttpContext());      // TcpConnection 与一个 HttpContext（上下文对象） 
...
void HttpServer::onMessage(TcpConnection* pCon, Buffer* pBuf, Timestamp receiveTime)
{
    ...
    WeakEntryPtr weakEntry(boost::any_cast<WeakEntryPtr>(pCon->getweakEntry()));
